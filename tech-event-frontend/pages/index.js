...
export default function Home() {
  ...
  // Fetch events if the user is logged in
  useEffect(() => {
    if (session) {
      axios
  ...
  const handleSelect = (event) => setSelectedEvent(event.target.value);

  if (status === 'loading') return <div>Loading...</div>;

  // Display welcome screen for logged-out users
  if (!session) {
    return (
      <div className="relative flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-900 to-purple-900 text-white overflow-hidden bg-cover bg-center" style={{ backgroundImage: "url('/background.png')" }}>
        ...
        {/* Login, Signup, and Community Page Buttons */}
        <div className="absolute top-5 right-5 z-10 flex space-x-4">
          ...
          <button
            className="bg-gradient-to-r from-indigo-500 to-purple-600 text-white py-2 px-4 rounded-lg font-poppins text-sm neon-effect shadow-lg hover:shadow-neon-blue hover:scale-110 transition duration-300"
  ...
  // Display events and greeting for logged-in users
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <main className="p-6">
        ...
        <button
          onClick={() => router.push('/communities')}
          className="mt-4 bg-gradient-to-r from-indigo-500 to-purple-600 text-white py-2 px-4 rounded-lg font-poppins text-sm neon-effect shadow-lg hover:shadow-neon-blue hover:scale-105 transition duration-300"
        >
